...
{"event":"agent_invoked","agent":"Interaction Designer","trigger":"designing form validation flow"}
{"event":"claim_consulted","claim_id":"claim-007","relevance_score":0.85}
{"event":"suggestion","suggestion_id":"sug-001","file":"docs/flows/checkout-flow.md","claim_refs":["claim-007"]}
{"event":"suggestion_outcome","suggestion_id":"sug-001","outcome":"accepted"}
{"event":"candidate_claim","description":"Inline validation on blur outperforms submit-time validation for forms with 5+ fields"}
{"event":"session_end","timestamp":"...","duration_minutes":25,"files_changed":["docs/flows/checkout-flow.md"]}
```
...
```bash
python -m scripts.epistemic.learn --plugin ux-pro
```
...
```
Tension detected:
  "claim-003 says always use progressive disclosure for complex forms, but this session
   showed all fields upfront for the same form type"
...
```
Observed pattern:
  "Inline validation on blur outperforms submit-time validation for forms with 5+ fields"
...
```
Needs attention:
  claim-012 (confidence 0.38): "Always use a stepper for multi-page forms"
...
```
[ux-pro]: N active claims, avg confidence X.XX, M items reviewed
```

If there were no review items, just print:

```
[ux-pro]: N active claims, avg confidence X.XX, session saved
```
