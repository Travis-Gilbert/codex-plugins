...
{"event":"agent_invoked","agent":"django-architect","trigger":"designing model hierarchy"}
{"event":"claim_consulted","claim_id":"claim-007","relevance_score":0.85}
{"event":"suggestion","suggestion_id":"sug-001","file":"essays/models.py","claim_refs":["claim-007"]}
{"event":"suggestion_outcome","suggestion_id":"sug-001","outcome":"accepted"}
{"event":"candidate_claim","description":"Cotton components should use c-vars for all prop defaults rather than template conditionals"}
{"event":"session_end","timestamp":"...","duration_minutes":25,"files_changed":["essays/models.py","templates/cotton/card.html"]}
```
...
```bash
python -m scripts.epistemic.learn --plugin django-design
```
...
```
Tension detected:
  "claim-003 says always use abstract base models for shared fields, but this session
   used multi-table inheritance for the same pattern"
...
```
Observed pattern:
  "Cotton components should use c-vars for all prop defaults rather than template conditionals"
...
```
Needs attention:
  claim-012 (confidence 0.38): "Always use select_related for ForeignKey traversals in templates"
...
```
[django-design]: N active claims, avg confidence X.XX, M items reviewed
```

If there were no review items, just print:

```
[django-design]: N active claims, avg confidence X.XX, session saved
```
