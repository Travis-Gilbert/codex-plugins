...
Before running your standard review process:

1. Read `knowledge/claims.jsonl` and filter for claims where domain starts with `django` (all subdomains).
2. For active claims with confidence > 0.8: treat them as authoritative additions to your checklist below.
3. For active claims with confidence 0.5–0.8: use them as advisory context.
4. If a claim contradicts a static checklist item below, and the claim is active with high confidence, prefer the claim — it has been validated through project use.
5. After completing your review, note which claims you applied and whether each was confirmed or contradicted by what you found.
